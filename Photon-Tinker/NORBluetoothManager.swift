...
class NORBluetoothManager: NSObject, CBPeripheralDelegate, CBCentralManagerDelegate {
    ...
    required init(withManager aManager : CBCentralManager) {
        ...
        UARTServiceUUID          = CBUUID(string: NORServiceIdentifiers.particleMeshServiceUUIDString)
        UARTTXCharacteristicUUID = CBUUID(string: NORServiceIdentifiers.particleMeshTXCharacteristicUUIDString)
        UARTRXCharacteristicUUID = CBUUID(string: NORServiceIdentifiers.particleMeshRXCharacteristicUUIDString)
        super.init()
...
//                    var builder = NSMutableString(bytes: buffer, length: MTU, encoding: String.Encoding.utf8.rawValue)
    ...
    func send(data aData : Data, withType aType : CBCharacteristicWriteType) {
        ...
        // The callback peripheral:didWriteValueForCharacteristic:error: is called only when the Write Request type was used,
        // but even if, the data is not available there.
        log(withLevel: .appLogLevel, andMessage: "\"\(aData.hexString)\" sent")
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        log(withLevel: .warningLogLevel, andMessage: "Particle Mesh commissioning Service not found. Try to turn bluetooth Off and On again to clear the cache.")
