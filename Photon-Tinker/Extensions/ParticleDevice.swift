...
extension ParticleDevice {
    private var pinsAssociatedObjectHandle: UInt8 = 0

    var pins:[DevicePin]? {
        set {
            objc_setAssociatedObject(self, &pinsAssociatedObjectHandle, value, .OBJC_ASSOCIATION_RETAIN)
        }
        get {
            guard let value = objc_getAssociatedObject(self, &pinsAssociatedObjectHandle) as? [DevicePin] else {
                return nil
            }
            return value
        }
    ...
    func configurePins() {
        ...
        switch self.type {
            ...
            default:
                ...
                a4 = DevicePin(label:"A4", logicalName:"A4", side:.left, row:3, availableFunctions:[.all]) // (II) Analog write duplicated to value in D3 (mention in UI)
                a5 = DevicePin(label:"A5", logicalName:"A5", side:.left, row:2, availableFunctions:[.all]) // (I) Analog write duplicated to value in D2 (mention in UI)
                ...
                d3 = DevicePin(label:"D3", logicalName:"D3", side:.right, row:4, availableFunctions:[.digitalRead, .digitalWrite, .analogWrite]) // (II) Analog write duplicated to value in A3 (mention in UI)
                d4 = DevicePin(label:"D4", logicalName:"D4", side:.right, row:3, availableFunctions:[.digitalRead, .digitalWrite]) // (II) Analog write duplicated to value in A4 (mention in UI)
                d5 = DevicePin(label:"D5", logicalName:"D5", side:.right, row:2, availableFunctions:[.digitalRead, .digitalWrite])
                d6 = DevicePin(label:"D6", logicalName:"D6", side:.right, row:1, availableFunctions:[.digitalRead, .digitalWrite])
                d7 = DevicePin(label:"D7", logicalName:"D7", side:.right, row:0, availableFunctions:[.digitalRead, .digitalWrite])
                break
