...
enum MeshSetupBluetoothConnectionFactoryState {
    case Disabled
    case Ready
    case Scanning
...
protocol MeshSetupBluetoothConnectionFactoryDelegate {
...
class MeshSetupBluetoothConnectionFactory: NSObject, CBCentralManagerDelegate {
    
    //MARK: - Delegate Properties
    var delegate : MeshSetupBluetoothConnectionFactoryDelegate?
    fileprivate var state : MeshSetupBluetoothConnectionFactoryState = .Disabled
    ...
    var connections             : [MeshSetupBluetoothConnection]?
    let particleMeshServiceUUID : CBUUID = CBUUID(string: "6E400001-B5A3-F393-E0A9-E50E24DCCA9E")
    
    //MARK: - BluetoothManager API
    
    required init(delegate : MeshSetupBluetoothConnectionFactoryDelegate) {
        super.init()
    ...
    func scanAndConnect(to peripheralName : String) -> Bool {
        if self.state == .Ready {
     ...
     */
    func drop(connection : MeshSetupBluetoothConnection) {
        if let p = connection.peripheral {
...
//        self.delegate?.didUpdateState(state: central.state)
        ...
        if (self.state != .Ready) {
            self.delegate?.bluetoothConnectionFactoryError(error: "Bluetooth is disabled", severity: .Warning)
        }
    }
...
//        log(level: .debugLogLevel, message: "peripheral.discoverServices([\(particleMeshServiceUUID.uuidString)])")
        ...
        let newConnection = MeshSetupBluetoothConnection(bluetoothConnectionFactory: self, connectedPeripheral: peripheral)
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if let connectionsArray = self.connections {
            for connectionElement in connectionsArray {
                if connectionElement.peripheral == peripheral {
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        ...
        delegate?.bluetoothConnectionFactoryError(error: "Failed to connect to bluetooth peripheral", severity: .Error)
    }
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if peripheral.name == self.peripheralNameToConnect {
            if RSSI.int32Value < -90  {
                ...
                self.delegate?.bluetoothConnectionFactoryError(error: "Device is too far from phone, get closer with your phone to the setup device", severity: .Warning)
//                self.delegate.message(...)
            ...
            if peripheral.state == .connected {
                ...
                self.delegate?.bluetoothConnectionFactoryError(error: "Reinitializing connection to device", severity: .Warning)
